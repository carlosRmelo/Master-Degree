...
inc = 92.99                                                   #Inclination [deg]
mbh =  10**(8.47)*u.solMass                                        #Mass of black hole [M_sun]
beta0 = np.array([0.29, 0.19, 0.46, -1.83, 0.39, 0.01, 0.11])                #Anisotropy parameter, one for each gaussian component

ML0 = np.array([9.58, 9.58, 8.13, 5.20, 4.93, 4.72, 4.50])*u.solMass/u.solLum       #Mass-to-light ratio per gaussian [M_sun/L_sun]
...
qDM_dat = np.ones_like(qDM_dat)*0.62                                                              #axial ratio of DM halo
...
Jampy_model.DM_component(surf_dm=10**(9.28)*surf_DM_dat.value, sigma_dm=sigma_DM_dat_ARC.value, qobs_dm=qDM_dat)
