...
#-----------------------------------------------------------------------------------------------------#
#Class for pseudo Elliptical NFW

class pseudoEllipticalNFW(SphericalNFW):
    @af.map_types
    def __init__(
        self,
        centre: dim.Position = (0.0, 0.0),
        ...
        """
        The pseudo Elliptical NFW profile, used to fit the dark matter halo of the lens.
        ...
        scale_radius : float
            ...
            the Universe..
        q: float
            Axial ratio of the potential. 
            (q = b/a = x_2/x_1 -> assuming a or x_1 the major axis along the x-axis)
        """

        super(pseudoEllipticalNFW, self).__init__(
            centre=centre,
            kappa_s=kappa_s,
            scale_radius=scale_radius,
        )
    ...
    def elliptical_radii(self, grid, a1e, a2e):
        """Convert a grid of (y, x) = (x_2, x_1) coordinates to a grid of their elliptical radii.
        ...
        a1e, a2e: floats
            Parameters of pseudo ellipitcal models.
        """

        return np.sqrt(np.add(a2e*np.square(grid[:, 0]), a1e*np.square(grid[:, 1])))
    ...
    def deflections_from_grid(self, grid, **kwargs):
        """
        Calculate the deflection angles at a given set of arc-second gridded coordinates.
        Here we assume the parametrization (14) of [1] for the parameters a1e and a2e.
        ...
        phie = np.arctan2(grid[:,0]*np.sqrt(a2e), grid[:,1]*np.sqrt(a1e)) 
        cos_phie, sin_phie = np.cos(phie), np.sin(phie)
        
        eta = np.multiply(1.0 / self.scale_radius, self.elliptical_radii(grid=grid, a1e=a1e, a2e=a2e))
        ...
        return np.multiply(deflection_grid[:, None], 
                                np.vstack((np.sqrt(a2e)*sin_phie, np.sqrt(a1e)*cos_phie)).T)
