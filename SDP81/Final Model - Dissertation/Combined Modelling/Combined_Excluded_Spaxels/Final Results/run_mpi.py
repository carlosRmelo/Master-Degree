...
data_folder = "/home/carlos/Documents/GitHub/Master-Degree/SDP81/Autolens/Combined/Data"#Reading MGE inputs
...
with MPIPool() as pool:
    ...
    surf_dm, sigma_dm = mgeNFW.fit(kappa_s=0.075,rs=1.0,ngauss=15,outer_slope=2, quiet=False)
    ...
    with open(f'sdp.pickle','rb') as f:
        sample_load = pickle.load(f)
        f.close()
    

    # Setting Configs for continue the sampling
    ...
    print(sample_load.sampling, sample_load.walks)
    ...
    while delta_logz > 0.1:
        ...
        delta_logz = resume_dlogz(sample_load)
        sample_load.add_final_live(print_progress=False)
        sampler_pickle = sample_load
        sampler_pickle.loglikelihood = None

        with open(f"new_sdp.pickle", "wb") as f:
        ...
        # Performing Update
        original = r"new_sdp.pickle"
        beckup   = r"beckup/new_sdp_beckup.pickle" 

        beckup = shutil.copyfile(original, beckup)
        log_table = np.loadtxt("Log.txt")
        run_time = time.time() - start
        np.savetxt("Log.txt",
                    np.vstack([log_table,np.array([sample_load.results.niter, sample_load.results.ncall.sum(), delta_logz, run_time])]),
                    header="Maxiter \t Maxcall \t dlogz \t Time[s]", 
                    fmt="%d \t\t %d \t\t %f \t %f")
        
        print(f"\nniter: %d. ncall: %d. dlogz: %f." %(sample_load.it, sample_load.ncall,delta_logz))

    

    print(f"\nSaving Final Sample:")
    with open(f"final_dynesty_lens.pickle", "wb") as f:
        pickle.dump(sample_load, f, -1) 
    print(f"\nSaved!!")     
