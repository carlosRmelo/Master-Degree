...
def integral_x(yrad, xrad, M0, sigma0, q0):
    return integrate.quad(alpha_x, 0, 1, args=(yrad, xrad, M0, sigma0, q0))

def integral_y(yrad, xrad, M0, sigma0, q0):
    return integrate.quad(alpha_x, 0, 1, args=(yrad, xrad, M0, sigma0, q0))
...
class MGE(geometry_profiles.SphericalProfile, mp.MassProfile):
    @af.map_types
    def __init__(
        self, centre: dim.Position = (0.0, 0.0)
        ):
        """
        Represents a MGE.
    ...
    def deflections_from_grid(self, grid):
        ...
        for i in range(len(grid)):
            ...
            grid_def[i,1] = ((const_factor*integrate.quad(alpha_x, 0, 1, args=(yrad.value, xrad.value, self.M, self.sigma, self.q))[0]*(u.solMass*u.rad)).to(u.arcsec)).value
        ...
        loomx = ThreadLoom(max_runner_cap=len(grid))
        loomy = ThreadLoom(max_runner_cap=len(grid))

        grid_rad = (grid*u.arcsec).to(u.rad)
        ...
        for i in range(len(grid)):
            ...
            loomx.add_function(integral_x, [yrad.value, xrad.value, self.M, self.sigma, self.q], {})
            loomy.add_function(integral_y, [yrad.value, xrad.value, self.M, self.sigma, self.q], {} )

        output_x = loomx.execute()
        output_y = loomy.execute()

        for i in range(len(grid)):
            grid_def[i,0] = (const_factor.value*output_x[0].get('output')[0]*u.rad).to(u.arcsec).value
            grid_def[i,1] = (const_factor.value*output_y[0].get('output')[0]*u.rad).to(u.arcsec).value
        '''
        return grid_def
    @property
    def is_MGE(self):
