"""
Attention!
This code runs in Python Multiprocessing mode.
"""
...
with Pool(7) as pool:
    ...
    sampler1 = emcee.EnsembleSampler(nwalkers, ndim, model, pool=pool,
    ...
    start = time.time()
    state = sampler1.run_mcmc(pos, nsteps=burnin, progress=True)
    print("\n")
    ...
    #Initialize the sampler
    sampler2 = emcee.EnsembleSampler(nwalkers, ndim, model, pool=pool,
                                     backend=backend, moves=moves)
    ...
    print("Burn in with %i steps"%burnin)
    start = time.time()
    state = sampler2.run_mcmc(pos, nsteps=burnin, progress=True)
    print("\n")
    print("Burn in elapsed time:", time.time() - start)
    sampler2.reset()
    print("\n")
