...
class MGE(geometry_profiles.SphericalProfile, mp.MassProfile):
    ...
    def deflections_from_grid(self, grid, quiet=True):
        ...
        if self.method == "sciquad":
            ...
            for i in range(len(grid)):                      
                result_x[i] = quadva(alphax, [0., 1.], args=(grid[i][0], grid[i][1], self.Total_Mass, self.Total_sigma_RAD, self.Total_qobs),epsrel=self.epsrel, epsabs=self.epsabs)   #Deflection in x


                result_y[i] =  quadva(alphay, [0., 1.], args=(grid[i][0], grid[i][1], self.Total_Mass, self.Total_sigma_RAD, self.Total_qobs),epsrel=self.epsrel, epsabs=self.epsabs)   #Deflection in y
